...
test -f ./metadata.json && grep -q '^  "name": "talend-cloudwatchlogs",$' ./metadata.json
if [ $? -ne 0 ]; then
  fError "ERROR: You must be inside the directory puppet-cloudwatchlogs to execute this script
  => cd ./puppet-cloudwatchlogs"
fi
